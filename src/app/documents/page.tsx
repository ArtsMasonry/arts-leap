...
export default function DocumentsPage() {
  return (
    <Suspense fallback={<main className="p-6"><h1 className="text-2xl font-bold">Documents</h1><p className="text-gray-600">Loading…</p></main>}>
      <DocumentsClient />
    </Suspense>
  );
}
          ...
          {docs.map(d => (
            <div key={d.id} className="rounded-2xl border bg-white p-4 shadow-sm">
              <div className="flex items-start justify-between">
                <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700">{d.type}</span>
                <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700">{d.status}</span>
              </div>
              <div className="mt-3">
